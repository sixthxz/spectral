...
## Proposition 6 — Log-Modulus Diagnostics Are Numerical, Not Theorem-Level Zero Criteria

### Numerical-diagnostic statement

For visualization only, on points where:
...
only in the extended-real visualization sense.
...
A numerical scan that does not detect off-line features does not prove that no off-center nontrivial zeros exist.

Without rigorous error bounds, the diagnostic carries no zero-existence or zero-nonexistence conclusion.

### Ledger-level annotation

The simulations helped motivate numerical-diagnostic labels, but those labels are provisional and carry no theorem-level content.

Use controlled vocabulary when making propositions:
...
coordinate location
zero-status
off-center zero-status
log-modulus diagnostic
```

### Guardrail

Numerical diagnostics guide exploration only. They do not establish RH-level claims.

---

## Proposition 7 — The Ledger Identifies a Framing Distinction, Not a Proof Step

### Ledger-level background

The ledger records the following historical/formal sequence as background context:
...
No theorem-level inference is made from this sequence.

### Ledger-level annotation

The ledger tracks distinct roles of `s`, including use as:

```text
exponent
convergence parameter
complex coordinate
input paired by the involution s -> 1-s
centered coordinate s = 1/2 + w
coordinate location
```

Zero-status remains a separate predicate:
...
The ledger marks possible category mistakes between:
...
It does not imply that off-center nontrivial zeros are impossible.

### Guardrail

The ledger identifies a framing distinction. A proof of RH would still require a theorem-level argument excluding off-center nontrivial zeros.
...
Using also the standard conjugation symmetry:

```text
xi(conjugate(s)) = conjugate(xi(s))
```

a zero `rho` gives zeros among:

```text
{rho, 1-rho, conjugate(rho), 1-conjugate(rho)}
```

with possible coincidences.
...
```text
zero set invariant under s -> 1-s
```
...
which is setwise invariant under s -> 1-s
```

### What remains unresolved

RH requires a theorem-level argument, specific to `xi`, excluding every off-center nontrivial zero:
...
Involution invariance alone does not provide that exclusion.

### Guardrail

This exclusion is not supplied by:
...
This interpretation helps audit whether critical-line parametrization is being assumed rather than proved.
It does not by itself prove that all nontrivial zeros lie on the critical line.
```

---

## Scope Note Before the Next Constructive Stage

Future theorem-level structures are outside the scope of these propositions and should be introduced only after separate review.

The next constructive stage should begin from the repaired theorem-level gap:

```text
prove, by theorem-level arguments specific to xi, that no off-center nontrivial zero
rho = beta + i gamma, 0 < beta < 1, beta != 1/2, exists.
```

These propositions do not identify that additional theorem-level structure. They only delimit what the current ledger, symmetry, analytic-structure, derivative, and numerical-diagnostic layers do and do not establish.
